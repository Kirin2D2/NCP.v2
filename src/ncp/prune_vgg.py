...
Unified pruning + fine-tuning pipeline for both vanilla VGG16 and AugmentedVGG16 (CNP).
...
instantiate the adapter and (a subclass of) PruningFineTuner, then call prune().
CNP runs use AugmentedVGGAdapter; vanilla LRP pruning uses VanillaVGGAdapter.
"""
...
class PruningFineTuner:
    ...
    def __init__(self, args, model: nn.Module, adapter: VGGAdapterBase):
        ...
        # Per-(group × target) subgroup stats (populated by test(compute_subgroup=True))
        # Each entry is a dict with keys: acc_g{g}y{y}, n_g{g}y{y},
        # precision_g{g}, recall_g{g}   (g=0/1 group, e.g. no-watermark/watermark; y=0/1 class)
    ...
    def setup_dataloaders(self):
        ...
        if data_type == 'basketball_imagenet':
            train_dataset, eval_dataset = dataset.get_basketball_imagenet(
                root_dir=self.args.basketball_root)
        else:
        ...
        self.train_loader = torch.utils.data.DataLoader(
            ...
            shuffle=True, **kwargs)
        # test_loader / eval_loader: same DataLoader. Watermark datasets yield (x, y, g);
        # test() uses g only for subgroup statistics.
    ...
    def test(self, compute_subgroup: bool = False):
        ...
        compute_subgroup: if True, also accumulate per-(group × target) subgroup
          statistics in the same pass (no extra data reads).  Only meaningful for
          data_types whose batches yield (x, y, g), e.g. watermark_imagenet (g = watermark).  Results are
        ...
        _ap_scores, _ap_labels, _ap_groups = [], [], []  # for per-group AP

        with torch.no_grad():
            for batch in self.test_loader:
                data, target = batch[0], batch[1]
                group = batch[2] if _do_sg and len(batch) > 2 else None
                if self.args.cuda:
        ...
        if _do_sg and sg_total:
            ...
            all_groups = sorted({k[0] for k in sg_total})
            for g in all_groups:
            ...
            if _ap_scores:
                ...
                _gn = torch.cat(_ap_groups).numpy()
    ...
    def train_batch(self, optimizer, batch_idx, batch, label, rank_filters: bool):
        ...
        if rank_filters:
            if self.args.relevance:   # LRP-based ranking
                ...
                # Relevance is seeded at the ground-truth label. The paper ranks filters w.r.t.
                # the positive logit; these coincide because the rank loaders used for CNP
                # contain only positive-class images.
