...
Purpose
-------
Implements the “virtual concept layer” augmentation used by CNP (Concept-aware Network Pruning)
for VGG16 at conv4_3, following the DRSA virtual-layer idea (Chormai et al., 2024): rewrite
conv4_3 activations into a concept-coordinate basis and back, optionally ablating selected
concept subspaces.
...
Ablation
--------
ablate_subspace_matrix(U, subspace_dims, irrelevant_subspaces) suppresses selected concept blocks
in both U and U^T. The paper describes ablation as setting U_k' = 0; this implementation scales
the blocks by 1e-4 instead (see the comment in ablate_subspace_matrix), so downstream layers see
approximately a_hat = U_ab U_ab^T a (a projection with those directions removed).

Subspace indices here are 0-indexed; the paper numbers subspaces from 1
(e.g. paper "subspace 4" == index 3).

Model structure
---------------
- Loads torchvision VGG16 (ImageNet weights by default).
...
Used by
-------
- experiments/run_watermark_pruning_experiment.py and experiments/run_PFT_basketball.py
  to build augmented models with ablated concept subspaces.
- prune_vgg.py (AugmentedVGGAdapter) for LRP through the augmented layers.
"""
...
def load_u_matrix(path) -> torch.Tensor:
    """Load a DRSA projection matrix U from a .pt (tensor) or .npy file as float32.

    A 3-D array of shape (d, n_subspaces, subspace_dim), as produced by drsa-demo, is
    reshaped to (d, n_subspaces * subspace_dim).
    """
    path = str(path)
    ...
    U = torch.load(path, map_location='cpu', weights_only=True)
...
class AugmentedVGG16(nn.Module):
    def __init__(self, U: torch.Tensor, UT: torch.Tensor,
        ...
        Args:
            ...
            weights: torchvision VGG16 weights to start from; None skips the download
                     (e.g. when a checkpoint is loaded afterwards).
        """
        super().__init__()

        self.augmented = True

        # Load VGG16 and split its feature extractor
